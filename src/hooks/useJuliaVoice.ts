...
export function useJuliaVoice(life: JuliaLifeSnapshot | undefined, date: string) {
  ...
  const runCapture = useCallback(
    async (parsed: Extract<VoiceIntent, { kind: 'capture' }>) => {
      ...
      if (parsed.captureKind === 'water') {
        const msg = `Water gelogd — totaal ${res.waterL ?? '?'} L in Notion.`;
        setApiMessage(msg);
  ...
  const speakLastReply = useCallback(() => {
    const text = apiMessage || (intent?.kind === 'speak' ? intent.text : null);
    if (!text) return;
    if (!getTtsEnabled()) {
      setTtsEnabled(true);
      setTtsOn(true);
    }
    // bypass getTtsEnabled check race: speak after enabling
