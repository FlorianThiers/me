...
$agentName = "AutoAssistant_" + (Get-Date -Format "yyyyMMddHHmmss")
$body = @{
    name = $agentName
...
try {
    ...
    $response.Content | Out-File -FilePath "moltbook-registration.json" -Encoding utf8
    ...
    $responseBody | Out-File -FilePath "moltbook-registration.json" -Encoding utf8
    
    if ($statusCode -eq 409) {
        Write-Host ""
        Write-Host "Agent name already exists. Try a different name or check if you are already registered."
    }
}
